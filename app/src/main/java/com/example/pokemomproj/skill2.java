...
public class skill2 extends AppCompatImageView {
    ...
    public skill2(Context context,BotView botView, CharacterView characterView) {
        super(context);
        if(botView == null) {
    ...
    private void initHealEffectbot() {
        ...
        float characterX = botView.getCharacterX();
        float characterY = botView.getCharacterY();
        ...
        float skillY = characterY - 72 - botView.getHeight() / 2;
   ...
   private void initHealEffect() {
    ...
    public  void BotstartHealing(BotView botView) {

        if (botView.getCurrentMana() >= 65) {
            botView.reduceMana(65);
            botView.healHp(50);
            if (botView.getParent() instanceof ViewGroup) {
                ViewGroup parent = (ViewGroup) botView.getParent();
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                        RelativeLayout.LayoutParams.WRAP_CONTENT,
                        RelativeLayout.LayoutParams.WRAP_CONTENT
                );
                params.addRule(RelativeLayout.BELOW, botView.getId());
                parent.addView(this, params);
            } else {
                Log.e(TAG, "characterView has no parent! Cannot add heal effect.");
            }
